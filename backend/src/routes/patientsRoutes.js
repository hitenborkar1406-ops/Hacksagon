...
// GET /api/patients/:id — single patient (frontend getPatientById)
router.get("/:id", authRequired, async (req, res) => {
  ...
  try {
    ...
    if (role === "patient" && String(id) !== String(userId)) {
      return res.status(403).json({ success: false, error: "Forbidden" });
    }
    const user = await User.findById(id).select("_id email role assignedDoctorId");
